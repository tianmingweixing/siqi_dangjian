...
<form class="layui-form" style="margin-top: 10px">
    <div class="layui-form-item">
        <label class="layui-form-label label_width_100">党小组名称</label>
        <div class="layui-input-inline">
            <input type="text" id="name_search"  placeholder="党小组名称" autocomplete="off" class="layui-input">
        </div>
            ...
            switch(obj.event){
                ...
                case 'update':
                    if(data.length === 0){
                        ...
                        layer.msg('data[0].id');
                    window.location.href='/partyTeam/setPartyTeam?id='+data[0].id;
                    }
                    break;
                case 'delete':
                    if(data.length === 0){
        ...
        table.on('tool(test)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'detail'){
                layer.msg('查看操作');
                 ...
                 layer.msg('edit');
                 console.log(data);
                     window.location.href='/partyTeam/setPartyTeam?id='+data.id;
